...
<body leftmargin="0" topmargin="0" bgcolor="#efefef">
	...
	<%set rs = conn.execute("Select c.codigo, c.processo, id, apelido from contencioso.dbo.TabProcCont p, envolvidos, contencioso.dbo.TabCliCont c where envolvidos.usuario = '"&Session("vinculado")&"' and envolvidos.tipo = 'outraparte' and c.usuario = '"&Session("vinculado")&"' and c.tipo = 'outraparte' and envolvidos.id = c.envolvido and c.processo = p.id_processo and p.id_processo = '"&tplic(0,vid_processo)&"' order by apelido")
		rowcount = 0
		do while not rs.eof	
		rowcount = rowcount+1
		%>
			<% If Request.Querystring("imprimir") = "" then %><a href="javascript: top.mostra_exc('cli','<%=rs("codigo")%>','<%=processo%>','O','<%=rs("id")%>','frame_outraparte')" class="preto10<%=l_imp%>"><% If Request.Querystring("imprimir") = "" then %><%if vid_processo <> "" then %><img src="imagem/lixeira.gif" alt="Excluir" width="13" height="17" border="0"><%end if%></a><%end if%><%end if%>&nbsp;&nbsp;&nbsp;&nbsp;<a class=preto10 href=" javascript: abrirjanela('ver_env.asp?id=<%=rs("id")%>',530,240)"><%= rs("apelido") %></a><br>
		<%rs.movenext
		loop%>		
...
if (<%=rowcount%>>'0') {
top.document.all.frame_outraparte.style.height = <%= (rowcount*28)+1 %>;
}
else
{
top.document.all.frame_outraparte.style.height = 0;
}
